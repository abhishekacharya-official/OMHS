<%-- 
    Document   : register_hospital
    Created on : 10 Mar, 2017, 12:09:45 PM
    Author     : 1405299
--%>
...
<body>
  <div class="navbar navbar-inverse navbar-fixed-top" role="navigation" >
    ...
    <form id="msform" action="reg_hospital.jsp" method="post" onsubmit="return ValidateForm();">
  ...
  <fieldset>
    ...
    <%
        ...
        <%
            ...
            if (s3!=null){
        %>
        <div class="alert alert-danger " style="padding: 0px;border-radius: 3px;margin-bottom: 10px;width: 100%;box-sizing: border-box;font-size: 13px;"><h3 class="fs-subtitle"><%= s3 %></h3></div>
        <%}%>
        <%
            String s4=(String) request.getAttribute("err4");
            if (s4!=null){
        %>
        <div class="alert alert-danger " style="padding: 0px;border-radius: 3px;margin-bottom: 10px;width: 100%;box-sizing: border-box;font-size: 13px;"><h3 class="fs-subtitle"><%= s3 %></h3></div>
        <%}%>
    <input type="text" name="hid" placeholder="Enter your desired hospital id" required pattern="^[A-Za-z0-9_@]{5,15}$" title="alphanumeric letters with '@','_' (min 5 and max 15)" />
    <input type="email" name="email" placeholder="Email" required pattern="(?!(^[.-].*|[^@]*[.-]@|.*\.{2,}.*)|^.{254}.)([a-zA-Z0-9!#$%&'*+\/=?^_`{|}~.-]+@)(?!-.*|.*-\.)([a-zA-Z0-9-]{1,63}\.)+[a-zA-Z]{2,15}" title="invalid email" />
    ...
    <input type="password" name="cpass" placeholder="Confirm Password" required pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" title="invalid password (UpperCase, LowerCase, Number/SpecialChar and min 8 Chars)"/>
    <input type="text" name="hregno" placeholder="Hospital Registration No." required pattern="^[A-Za-z0-9]{5,15}$" title="alphanumeric letters(min 5 and max 15)" />
